...
<h1 class="text-center"> Edit Record </h1>
		...
		<form method="post" action="editpost.php">
			...
			<input type="fullname" value="<?php echo $dase['fullname'] ?>" id="fullname" name="fullname" class="form-control">
		</div>
			  <div class="mb-3">
			...
			<input type="password" value="<?php echo $dase['password'] ?>" id="password" name="password" class="form-control">
			<div id="passwordHelpBlock" class="form-text">A strong password is 8 characters or longer, contains at least 1 lowercase and uppercase letter, and contains at least a number or a symbol, or is 20 characters or longer.</div>
						  <div class="mb-3">
			...
			<input type="text" value="<?php echo $dase['dob'] ?>" id="dob" name="dob" class="form-control">
				...
				<div class="mb-3">
					...
					<?php while($r = $results->fetch(PDO::FETCH_ASSOC)) { ?>
                      <option value="<?php echo $r['profession_id']?>" <?php if($r['profession_id'] == $dase['profession_id']) echo 'selected'?>>
                      	<?php echo $r['name']; ?>
                  </option>
                     <?php } ?>
					</select>
